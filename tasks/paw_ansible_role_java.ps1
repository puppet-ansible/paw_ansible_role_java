...
if ($env:PT__installdir) {
  $AnsibleDir = Join-Path $env:PT__installdir "lib\puppet_x\ansible_modules\ansible_role_java"
} else {
  # Fallback to Puppet cache directory
  $AnsibleDir = "C:\ProgramData\PuppetLabs\puppet\cache\lib\puppet_x\ansible_modules\ansible_role_java"
}
...
$CollectionPlaybook = Join-Path $AnsibleDir "roles\paw_ansible_role_java\playbook.yml"
...
if ((Test-Path (Join-Path $AnsibleDir "roles")) -and (Test-Path $CollectionPlaybook)) {
  ...
  $PlaybookDir = Join-Path $AnsibleDir "roles\paw_ansible_role_java"
  ...
  $result = @{
    _error = @{
      msg = "playbook.yml not found in $AnsibleDir or $AnsibleDir\roles\paw_ansible_role_java"
...
$ExtraVars = @{}
if ($env:PT_java_home) {
  $ExtraVars['java_home'] = $env:PT_java_home
}
